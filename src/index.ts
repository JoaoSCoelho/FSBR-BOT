...
bot.on('interactionCreate', (interaction) => {
  ...
  if (interaction.message.id === '913784514000588870') {
    if (interaction.member.roles.cache.has(interaction.customId)) {
      ...
      interaction.reply({
        content: `Você deixou de ser um jogador de ${
          interaction.message.guild!.roles.cache.get(interaction.customId)!.name
        }`,
        ephemeral: true,
      })
    } else {
      ...
      interaction.reply({
        content: `Você agora é oficialmente um jogador de ${
          interaction.message.guild!.roles.cache.get(interaction.customId)!.name
        }`,
        ephemeral: true,
      })
